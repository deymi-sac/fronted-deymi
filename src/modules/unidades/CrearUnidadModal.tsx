...
import { useCrearUnidad } from "./useCrearUnidad";
...
export function CrearUnidadModal({ isOpen, onClose }: CrearUnidadModalProps) {
  const [formData, setFormData] = useState({
    uni_placa: "",
    uni_placa_secundaria: "",
    uni_type_vehiculo: "",
    id_estado: "",
    uni_observaciones: "",
  });
  ...
  const { mutate, isPending, error, reset } = useCrearUnidad();
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          setFormData({ uni_placa: "", uni_placa_secundaria: "", uni_type_vehiculo: "", id_estado: "", uni_observaciones: "" });
          onClose();
        },
      }
    );
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                required
                placeholder="Ej. Tracto, Semi Trailer"
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
              />
            </div>
